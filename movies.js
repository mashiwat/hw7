firebase.auth().onAuthStateChanged(async function(user) {
  ...
  if (user) {
    ...
    let apiKey = '735e39fc5e75cf327e233666d31a2e63'
    ...
    for (let i=0; i<movies.length; i++) {
      let movie = movies[i]
      let docRef = await db.collection('watched2').doc(`${movie.id}-${user.uid}`).get()
      ...
      document.querySelector(`.movie-${movie.id}-${user.uid}`).addEventListener('click', async function(event) {
        ...
        await db.collection('watched2').doc(`${movie.id}-${user.uid}`).set({})
      }) 
    }

    let userName = user.displayName
    
    document.querySelector('.sign-in-or-sign-out').innerHTML = ` <div class="md:mx-0 mx-4">
